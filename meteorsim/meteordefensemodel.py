...
class MeteorEventScheduler:
    def __init__(self, num_cannons: int):
        self.tick = 0
        self.prev_tick = 0
        self.event_queue = []
...
def main():
    ...
    event_scheduler = MeteorEventScheduler(NUM_CANNONS)
    event_idx = 0
    ...
    while event_scheduler.tick <= MAX_SIM_TICKS:
        ...
        nxt_cannon_idx = event_scheduler.get_nxt_ready_cannon_idx()

        while nxt_cannon_idx > -1:
            if num_meteors_outstanding > 0:
                ...
                event_scheduler.cannon_ready_tick[nxt_cannon_idx] = event_scheduler.tick + RECHARGE_TICKS
